...
export default function MobileNavbar() {
    ...
    return (
        <div className="tab-mobile-container">
            <Tab.Container className="tab-mobile-wrapper" activeKey={activeKey} onSelect={setActiveKey}>
                <Nav className="navs-mobile-container d-flex justify-content-center col-12 mr-3 mr-xs-0">
                    {navs.map(nav => {
                        return (
                            <Nav.Item className="nav-mobile-container" key={nav.id}>
                                <Nav.Link eventKey={`${nav.title}`} className="nav-title">{nav.title}</Nav.Link>
                            </Nav.Item>
                        )
                    })}
                </Nav>
                <Tab.Content className="col-12 border-top pt-3" style={{textAlign: 'center'}}>
